...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  request.requestBody({
    ...
    // Optional: Add application_context for return and cancel URLs if needed for a redirect flow
    // application_context: {
  ...
  try {
    ...
    if (error.isAxiosError && error.response && error.response.data && error.response.data.message) { // This check might need adjustment based on the SDK's error structure

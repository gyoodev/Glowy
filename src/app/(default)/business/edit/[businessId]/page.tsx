                  ...
                  {serviceFields.map((item, index) => (
                    <Card key={item.id} className="p-4 space-y-3 relative">
                       <div className="space-y-1">
                        ...
                        <Popover open={openServicePopovers[index]} onOpenChange={(isOpen) => setOpenServicePopovers(prev => ({...prev, [index]: isOpen}))}>
                            <PopoverTrigger asChild>
                                <Controller
                                    ...
                                    render={({ field }) => (
                                        <Button
                                            variant="outline"
                                            role="combobox"
                                            aria-expanded={openServicePopovers[index]}
                                            className="w-full justify-between font-normal"
                                        >
                                            {field.value || "Изберете или въведете име"}
                                            <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                                        </Button>
                                    )}
                                />
                            ...
                            <PopoverContent className="w-[--radix-popover-trigger-width] p-0">
                                <Command shouldFilter={false}> {/* We handle filtering and custom input */}
                                    <Controller
                                        name={`services.${index}.name`}
                                        control={form.control}
                                        render={({ field }) => (
                                            <CommandInput 
                                                placeholder="Търсене или ново име..."
                                                value={field.value}
                                                onValueChange={(currentValue) => {
                                                    field.onChange(currentValue); // Update form state
                                                    // If user clears input or types something not in mockServices, potentially clear other fields or set custom ID
                                                    const matchedService = mockServices.find(ms => ms.name.toLowerCase() === currentValue.toLowerCase());
                                                    if (!matchedService) {
                                                        form.setValue(`services.${index}.id`, `custom_${Date.now()}_${index}`, { shouldValidate: true });
                                                        // Optionally clear description, price, duration if you want them to be re-entered for custom services
                                                        // form.setValue(`services.${index}.description`, '', { shouldValidate: true });
                                                        // form.setValue(`services.${index}.price`, 0, { shouldValidate: true });
                                                        // form.setValue(`services.${index}.duration`, 30, { shouldValidate: true });
                                                    }
                                                }}
                                            />
                                        )}
                                    />
                                    <CommandList>
                                        <CommandEmpty>Няма намерени услуги. Можете да въведете ново име.</CommandEmpty>
                                        <CommandGroup>
                                            {mockServices.filter(ms => ms.name.toLowerCase().includes(form.watch(`services.${index}.name`)?.toLowerCase() || '')).map(service => (
                                                <CommandItem
                                                    key={service.id}
                                                    value={service.name}
                                                    onSelect={() => {

...
// Footer import is removed from here as it's handled by RootLayout
...
export default function SalonDirectoryPage() {
  ...
  useEffect(() => {
    const fetchSalons = async () => {
      ...
      try {
        ...
        // Order by promotion status first, then by rating, then by name
        // This query requires a composite index: promotion.isActive (desc), rating (desc), name (asc)
        // Or, more simply for now, fetch all and sort client-side if Firestore indexing is complex.
        // For client-side sorting after fetch:
        const q = query(salonsCollectionRef, orderBy('name')); // Basic query, sorting will be done client-side
        ...
        // Client-side sorting: promoted first, then by rating (desc), then by name (asc)
        salonsList.sort((a, b) => {
          ...
          // If both have same promotion status, sort by rating (desc)
          if ((a.rating ?? 0) !== (b.rating ?? 0)) {
            return (b.rating ?? 0) - (a.rating ?? 0);
          }
          
          // If ratings are same, sort by name (asc)
  ...
  const applyFilters = useCallback(() => {
    let tempSalons = [...salons]; // Start with all fetched (and presorted) salons
